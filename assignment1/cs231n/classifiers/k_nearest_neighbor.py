...
class KNearestNeighbor:
  ...
  def compute_distances_no_loops(self, X):
    ...
    dists = np.repeat(np.sum(X**2, axis=1), num_train).reshape(num_test,num_train) +\
            np.repeat(np.sum(self.X_train**2, axis=1), num_test).reshape(num_train,num_test).T-\
  ...
  def predict_labels(self, dists, k=1):
    ...
    for i in xrange(num_test):
      ...
      # label.                                                                #
      #########################################################################
      candidate_y = list(self.y_train[closest_y])

      res = np.zeros(k)
      for j in range(k):
          res[j] = candidate_y.count(candidate_y[j])

      y_pred[i] = candidate_y[np.argmax(res)]
      #########################################################################
      #                           END OF YOUR CODE                            # 
      #########################################################################
